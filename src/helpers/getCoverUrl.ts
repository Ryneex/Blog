 ...
 */
export async function getCoverUrl(cover: string | File): Promise<ITypeGetCoverUrl> {
    if (typeof cover === "string") return { success: true, url: cover }

    const res: ITypeGetCoverUrl = await new Promise(async (res) => {
        const stream = cloudinary.v2.uploader.upload_stream({ resource_type: "image", folder: "/blog/cover" }, (err, data) => {
