...
export function CodeBlockCombobox() {
    ...
    const items = languages.filter((language) => !value || language.label.toLowerCase().includes(value.toLowerCase()) || language.value.toLowerCase().includes(value.toLowerCase()))

    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button size="xs" variant="ghost" className="h-5 justify-between px-1 text-xs" aria-expanded={open} role="combobox">
