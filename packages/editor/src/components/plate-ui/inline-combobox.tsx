import { type HTMLAttributes, type ReactNode, type RefObject, createContext, forwardRef, startTransition, useCallback, useContext, useEffect, useMemo, useRef, useState } from "react"

import type { PointRef } from "slate"

import { type ComboboxItemProps, Combobox, ComboboxItem, ComboboxPopover, ComboboxProvider, Portal, useComboboxContext, useComboboxStore } from "@ariakit/react"
    ...
    return (
        <>
            ...
            <span className="relative min-h-[1lh]">
                ...
                <Combobox ref={ref} className={cn("absolute left-0 top-0 size-full bg-transparent outline-none", className)} value={value} autoSelect {...inputProps} {...props} />
            </span>
        </>
    )
})
...
const InlineComboboxContent: typeof ComboboxPopover = ({ className, ...props }) => {
    ...
    return (
        <Portal>
            <ComboboxPopover className={cn("z-[500] max-h-[288px] w-[300px] overflow-y-auto rounded-md bg-white shadow-md dark:bg-slate-950", className)} {...props} />
        </Portal>
    )
}

const comboboxItemVariants = cva("relative flex h-9 select-none items-center rounded-sm px-2 py-1.5 text-sm text-slate-950 outline-none dark:text-slate-50", {
    defaultVariants: {
        interactive: true,
    },
    variants: {
...
const InlineComboboxItem = ({ className, keywords, onClick, ...props }: InlineComboboxItemProps) => {
    ...
    const visible = useMemo(() => !filter || filter({ keywords, value }, search as string), [filter, value, keywords, search])
